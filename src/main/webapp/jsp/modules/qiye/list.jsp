...
<body>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('qiyeguimo')">企业规模<i id="qiyeguimoIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('qiyejianjie')">企业简介<i id="qiyejianjieIcon" class="fa fa-sort"></i></th>
											<th>操作</th>
										</tr>
									</thead>
									<tbody>
	...
	<script>
		...
		function setDataRow(item,number){
			...
			var qiyeguimoCell  = document.createElement('td');

						 qiyeguimoCell.innerHTML = item.qiyeguimo;


			row.appendChild(qiyeguimoCell);
			...
			var qiyetupianCell  = document.createElement('td');

				                var qiyetupianImg = document.createElement('img');
			qiyetupianImg.setAttribute('src',item.qiyetupian);
			qiyetupianImg.setAttribute('height',100);
			qiyetupianImg.setAttribute('width',100);
			qiyetupianCell.appendChild(qiyetupianImg);


			row.appendChild(qiyetupianCell);
			...
			var qiyejianjieCell  = document.createElement('td');

						  //如果图文详情包含图片
			  if(item.qiyejianjie && item.qiyejianjie.indexOf('img') != -1){
				//暂时只考虑图片+文字 和 文字+图片的情况
				var beginIndex = item.qiyejianjie.indexOf('<img');
				var endIndex = item.qiyejianjie.indexOf('>');				
				qiyejianjieCell.innerHTML = item.qiyejianjie.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
			  }else{
			  	 if(item.qiyejianjie != null && item.qiyejianjie != "" && item.qiyejianjie.length >= 11){
			  		qiyejianjieCell.innerHTML = item.qiyejianjie.substring(0, 10) + "..."; 
			  	}else{
			  		qiyejianjieCell.innerHTML = item.qiyejianjie;
			 	}
			  }
				  


			row.appendChild(qiyejianjieCell);
