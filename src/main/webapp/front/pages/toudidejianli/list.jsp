		...
		<div id="app">
			...
			<div class="data-container layui-row">
				...
				<fieldset class="search-container">
					<form class="layui-form">
						...
						<div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
							...
							<button v-if="isAuth('toudidejianli','新增')" @click="jump('../toudidejianli/add.jsp')" type="button" class="layui-btn btn-theme">
				...
				<div class="data-list layui-col-md8 layui-col-md-offset2">
					<div @click="jump('../toudidejianli/detail.jsp?id='+item.id)" v-for="(item,index) in dataList" v-bind:key="index"
					 class="data-item layui-col-md3">
						
						                                                                                                                                                                                                                                                                                                                                                <img :src="item.zhaopian?item.zhaopian.split(',')[0]:''" class="cover">
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                						
																								                        <h3 class="title">{{item.xueshengxingming}}</h3>
				...
				function pageList() {
					var param = {
						...
						param['xueshengxingming'] = jquery('#xueshengxingming').val() ? '%' + jquery('#xueshengxingming').val() + '%' : '';
                                                                                ...
                                                                                if (jquery('#xueli').val()) {
						param['xueli'] = jquery('#xueli').val() ? '%' + jquery('#xueli').val() + '%' : '';
					}  
                                                                                if (jquery('#zhuanye').val()) {
						param['zhuanye'] = jquery('#zhuanye').val() ? '%' + jquery('#zhuanye').val() + '%' : '';
					}  
                                                                                                                                                                                                                                                                                        if (jquery('#qiuzhigangwei').val()) {
						param['qiuzhigangwei'] = jquery('#qiuzhigangwei').val() ? '%' + jquery('#qiuzhigangwei').val() + '%' : '';
					}  
                                                                                                                                                                                                                                                                                                            
					// 获取列表数据
					http.request('toudidejianli/list', 'get', param, function(res) {
